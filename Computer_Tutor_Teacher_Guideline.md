...
* **Step-by-Step Exercise Facilitation Guide**:
  ...
  * **How to do it**:
    ...
    2. Add the `Scan for target warehouse door` block to position 2.
    3. Add the `Fly drone to the door` block to position 3.
    4. Add the `Use digital keycard to unlock` block to position 4.
    5. Click the **Run Sequence** button. The drone will boot, scan, fly to the door, unlock it, and display a "Success" message.
  * **Purpose of the Exercise**:
    - To introduce the student to the concept of **sequential processing** (how computers execute lines of code chronologically from top to bottom) and **hardware preconditions** (the drone cannot scan or move without the power source being booted first).
  * **What the Student Learns**:
    ...
    - The difference between active system states and dormant system states.
  * **Tutor Check Question**: *"Why did we need to turn the power on first? What happens if you try to scan a door with the power offline?"*

  ##### **Exercise 1.2: Variable Coordinates**
  * **How to do it**:
    1. Sequence the blocks in the same logical order: `Turn on drone power` ➔ `Scan for target warehouse door` ➔ `Fly drone to the door` ➔ `Use digital keycard to unlock`.
    2. Explain that in this exercise, the `Scan` block specifically reads the door coordinates and stores them in a memory variable named `targetCoords`.
    3. When the `Fly` block runs, it dynamically references the value stored inside `targetCoords` to navigate the drone.
    4. Click **Run Sequence** to execute.
  * **Purpose of the Exercise**:
    - To demonstrate **variables as data storage containers** and explain **data dependency** (Step $B$ cannot run if it relies on data generated by Step $A$, even if the power is on).
  * **What the Student Learns**:
    - A variable is a temporary memory slot that must be loaded/defined before it can be read.
    - If the drone tries to fly to `targetCoords` before scanning, `targetCoords` is `null` (empty), leading to an immediate navigation crash.
  * **Tutor Check Question**: *"How does scanning the door help the flying step? What variable is loaded in memory during the scan?"*
