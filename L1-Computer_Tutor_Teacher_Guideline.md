...
>
> **Grading note (2026-07-30, Sessions 1, 5, 6; extended 2026-08-05 to Sessions 8, 9, 10, 11, 12; extended again 2026-08-21 to Session 7):** all twelve sessions' exercises and Project Tasks are now graded by a real AI Auditor (DeepSeek). Their instructions were also rewritten to state the **goal**, not the answer: a student is asked "what should this achieve" and expected to plan and prompt in their own words, rather than being told the exact code/logic to ask for. Session 7 was the last holdout — it had kept the original keyword-checked format even after the rest of Level 1 moved on, which a real student hit as "guessing the exact wording" rather than being graded on actual prompt quality; see Session 7's own Content note below for what changed. Alongside this, an **optional "🤖 Generate Code" / "🤖 Generate Answer"** button now sits next to "Copy Prompt" in every session (including 7, added 2026-08-20) — it calls the same AI server-side as a quick in-platform check, but "Copy Prompt" and the requirement to actually practice with a real AI IDE (ChatGPT/Cursor/Copilot) are unchanged and still the primary path.
>
...
**Format note (updated 2026-07-30):** Session 4 has **4 exercises** (the old 5th, "The Complete Variable Registry," was dropped as a duplicate of work the Project Task already assumes — see below), each with **3 input boxes**: (1) **Plan & Design**, (2) **Writing Prompt** and **Output Code** side by side, (3) **Explain the Output Code**. The student pastes the *real* code their AI tool generated into Output Code. **Verify grades the Prompt box only** — clarity, specificity, completeness, not exact wording (see the Lab Track's **Grading note** above). **Note on the live preview:** this session's code is variables & math only and never touches the DOM, so there's no racing-game graphic here — the preview shows a **Console Output** panel with whatever the code prints via `console.log` (a blank Console Output is not a failure — not every exercise needs to print something), and a separate **Terminal Log** panel below the Verify button shows the Auditor's feedback.
...
Students complete **4 exercises** (the old 5th "combine everything" capstone was dropped — see the Content note above), each with 3 boxes: **1) Plan & Design**, **2) Write the AI Prompt & Paste the Output Code**, **3) Explain the Output Code**. The Interactive Live Preview is the **Live Racing Game Preview** (click inside it, then press arrow keys to steer) — the student's pasted Output Code runs live against the real racing-game DOM, with a **Console Output** panel next to the preview and a separate **Terminal Log** panel below the Verify button showing the Auditor's feedback. **Verify grades the Prompt box only**, on prompt-writing quality (clarity, specificity, completeness) — not the Plan/Output Code/Explain boxes, and not whether it names any specific word/number.
...
Students complete **4 exercises** (the old 5th "combine everything" capstone was dropped — see the Content note above), each with 3 boxes: **1) Plan & Design**, **2) Write the AI Prompt & Paste the Output Code**, **3) Explain the Output Code**. The Interactive Live Preview is the Live Racing Game Preview — the pasted Output Code runs live and the guards can be tested by steering to both edges and by accelerating/braking (the preview seeds `carX`/`speed` variables and now also shows a live `#speed-val` HUD and a scrollable striped road background so 6.3/6.4's effects are visible, not just logged), with a **Console Output** panel next to the preview and a separate **Terminal Log** panel below the Verify button showing the Auditor's feedback. **Verify grades the Prompt box only**, on prompt-writing quality (clarity, specificity, completeness) — not the Plan/Output Code/Explain boxes, and not whether it names any specific word/number.
...
* **Exercise 7.4: The Danger Zone — For Loop + Conditional Road Color**
  * *Goal given to the student:* a `for` loop that **schedules** delayed speed updates via `setTimeout` (not a plain synchronous loop, which would resolve to the final speed instantly with nothing to watch), checking `speed` against the 100 danger threshold on each scheduled update and turning `#game-track`'s border red once it's reached (yellow otherwise). `speed` already exists in this sandbox starting at 0 (like `carX` already exists at 165) — the exercise text explicitly warns not to redeclare it.
  * *A strong answer shows:* a `for` loop scheduling several staggered `setTimeout` updates (each a bit later than the last), each one increasing the existing `speed` (not a redeclared one) and checking it with an `if/else` that sets `#game-track`'s border color; an explanation identifying which scheduled update (and speed value) actually crossed 100 and changed the color, and correctly saying a non-delayed for loop can't show this as a visible progression. (Found and fixed 2026-08-21: the first version of this exercise asked for a plain, non-delayed for loop — technically correct, but it resolved to the final speed the instant the code ran, so a student opening the Preview just saw speed already at its end value with no car and no visible acceleration to watch. Reworded to require delayed scheduling, and the car — hidden for 7.1-7.3 since they never move it — is shown for 7.4 specifically, since this exercise is about the car crossing a speed threshold.)
