...
The **Project Journal** milestone card ("Lab 6: Safety Guards & Boundary Clamps") tracks the student's own project script, extending the student's own Session 5 code — including its `steerCar()` function and ◀/▶ buttons. **Deliberately not a repeat of the sandbox exercises**: 6.1-6.3 guard a plain keydown handler with fixed numbers, and 6.4's overheat clamp is reused here rather than re-taught, so the Project Task's real differentiator is WHERE the guard lives — inside `steerCar()` itself, so it protects the keyboard AND the on-screen buttons at once, something no S6 exercise even mentions (none of them know the buttons exist). It uses the same 3-box shape: **1) Plan & Design**, **2) Prompt & Output Code** (with the live Racing Game Preview and an optional "🤖 Generate Code" quick-check button alongside "Copy Prompt"), **3) Explain the Code**.

1. **Plan & Design**
   - *Goal given to the student:* work out, in your own words, WHERE the boundary check has to live so it protects both input paths at once — not copied into the keydown handler and each button's click handler separately — and how to derive real left/right limits from your own `TRACK_WIDTH`/car width instead of the exercises' fixed 35/295.
   - *A strong answer shows:* recognizing that guarding only `keydown` would leave the buttons unprotected, so the check belongs inside `steerCar(direction)` itself — the one place both input paths already call — plus limits computed from the student's own `TRACK_WIDTH`.
   - *Why:* This is the first session where the design question is "where does shared logic live," not just "what should the rule be" — a direct continuation of Session 5's single-shared-action lesson.

2. **Prompt & Output Code**
   - *Goal given to the student:* write your own prompt for the guard you planned, making sure it tells the AI to add the check INSIDE the existing `steerCar()` function (not duplicated separately per input path), derive limits from the student's own `TRACK_WIDTH`/car width, and bring in the speed overheat clamp already built in Exercise 6.4.
   - *A strong answer shows:* Output Code with the boundary check living inside `steerCar()`, limits genuinely computed from the student's own `TRACK_WIDTH` (not a hardcoded 35/295), and the reused overheat clamp resetting speed to a Number, not a string. (An optional "🤖 Generate Code" button can fill this in as a quick in-platform check; "Copy Prompt" + a real AI tool is still the primary path.)
   - *Why:* Since Session 3 let students choose their own track width, a correct guard computes its limits from that width — and because it lives inside `steerCar()`, the AI Auditor and a quick button-click test can both verify it protects every input path, not just keyboard.

3. **Explain the Code**
   - *A strong answer shows:* the explanation names both halves — the limits are derived from the student's own `TRACK_WIDTH`/car width, and the check lives inside `steerCar()` specifically so clicking a button at the edge behaves identically to pressing the matching key.
   - *Expected Socratic answer* — *"If the guard were only added inside the keydown handler, what would happen if the player held the boundary lane with the keyboard, then clicked the ▶ button instead of pressing a key again?"* → The button's click handler calls `steerCar()` directly, bypassing the keydown handler entirely — so a guard placed only in `keydown` would never run, and the button click would push the car straight past the edge.

* **Homework Evaluation Checklist**: the student's Project Journal shows the boundary guard living inside `steerCar()` (not duplicated across input paths), both limits derived from their own `TRACK_WIDTH` (the AI Auditor's "🤖 Get AI Feedback" button flags a hardcoded/reinvented boundary directly), and the reused speed clamp resetting to a Number. Verify by clicking a button at the edge in the Live Preview — it should lock exactly like the matching arrow key does.
