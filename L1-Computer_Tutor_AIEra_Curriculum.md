...
>
> **Verify gate simplified (2026-08-06):** Verify used to also require Plan, Output Code, and Explain to be filled in before it would check the Prompt — a completion gate left over from before the grading redesign above. Real use showed this as confusing (a working prompt got blocked with "fill in all four boxes"), so the gate now only requires the Prompt box; Plan/Output Code/Explain remain present as practice fields, just no longer required to click Verify. Applies to S1-S6 and S8-S12 — **Session 7 is unaffected**, still keyword-graded across all four boxes.
