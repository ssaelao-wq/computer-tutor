...
>
> **Grading note (2026-07-30, Sessions 1-6; extended 2026-08-05 to Sessions 8-12; extended again 2026-08-21 to Session 7):** all twelve sessions are now graded by a real AI Auditor (DeepSeek), replacing the lightweight-keyword check. Session 7 was the last holdout — a real student hit its old keyword-checked format as "guessing the exact wording," since a genuinely well-written prompt phrased differently than the exercise's literal required substrings still failed outright; see Session 7's Tutor Manual content note below. Instructions now state the **goal**, not the answer, and every session also has an optional "🤖 Generate Code"/"🤖 Generate Answer" button next to "Copy Prompt" (a quick in-platform check; the external-AI-tool requirement is unchanged) — Session 7's button was added 2026-08-20. Sessions 2, 3, 4, 5, 6, 8, 9, 10, and 11 each dropped their old 5th "combine everything" capstone exercise (4 exercises, not 5) as a duplicate of that session's own Project Task. **Sessions 1, 7, and 12 keep all 5** — none of their 5th exercises duplicate their Project Task: Session 1's exercises were never a capstone-duplication pattern to begin with; Session 7's Ex 7.5 ("Complete Marker System") uses fixed example numbers, while its Project Task requires computing spacing from the student's own Session 3 track height; Session 12's Ex 12.5 is a personal reflection/whole-level QA sweep, not a rebuild of the CONFIG-refactor Project Task. Sessions 5 and 6 together also form one explicit capability milestone — *"the car remembers itself, responds to you, and stays safe."* The 2026-08-05 pass also fixed real bugs found by testing "🤖 Generate Code" against the live AI — see each of Sessions 8, 9, 11's Tutor Manual notes below for what broke and how it was fixed.
>
> **Grading redesign (2026-07-30, later same day):** real student testing showed grading all four boxes for correctness still failed genuinely well-written prompts whenever they targeted a different specific detail than intended (a clear, precise prompt still failed for naming the "wrong" key) — still felt like a wording check, not a measure of prompt-writing skill. **Verify now grades ONLY the Prompt box**, on clarity/specificity/completeness, with 2-3 concrete prompt-writing tips. Session 5's Exercises 5.2-5.4 were also rewritten the same day: 5.1's solution already covers both directions + DOM movement, so the original replacements had become redundant or hollow — the final versions are "Detecting All Four Directions," "Different Distances Per Direction," and "Pause with Spacebar."
>
> **Verify gate simplified (2026-08-06):** Verify used to also require Plan, Output Code, and Explain to be filled in before it would check the Prompt — a completion gate left over from before the grading redesign above. Real use showed this as confusing (a working prompt got blocked with "fill in all four boxes"), so the gate now only requires the Prompt box; Plan/Output Code/Explain remain present as practice fields, just no longer required to click Verify. Applies to every Auditor-graded session — as of the 2026-08-21 update above, that's now all of S1-S12, including Session 7.
...
**Content note (2026-08-21):** moved to real AI-Auditor grading and goal-stated instructions (see the Grading note above) — **Verify now grades the Prompt box only**, the last of Level 1's twelve sessions to make this move; a real student had been hitting the old keyword-checked format as guessing exact wording. Exercise count and content unchanged (still **5 exercises**, kept for the reason noted in the Grading note above). Also fixed: Exercises 7.1-7.3 produce no visible DOM output at all (pure computation/console.log), so their Live Preview now shows a Console Output panel instead of the racing-track graphic, which previously sat there with an idle car implying something should move; 7.4-7.5 (which really render `marker-dash` elements) keep the Live Racing Game Preview.

5 exercises, each with 3 boxes, goal-oriented instructions:

- **Exercise 7.1 (Marker Spacing Plan & the Loop)**: goal is a `for` loop running 5 times, computing `markerY` as `i * 120`; a strong answer shows a real `for(...i<5...i++)` loop with `markerY = i * 120`, and an explanation naming all 3 loop-header parts.
- **Exercise 7.2 (A Second Spec — 8 Distance Markers, 90px Apart)**: goal is a second, independent `for` loop running 8 times, computing `markerY` as `i * 90` (different numbers than 7.1, so it can't be copied); a strong answer shows a real `for(...i<8...i++)` loop with `markerY = i * 90`, and an explanation naming all 3 loop-header parts AND what happens to the browser if the update part were ever left out.
- **Exercise 7.3 (Logging Each Marker)**: goal is a `console.log` of `markerY` inside the loop; a strong answer shows a real `console.log` each pass, and an explanation correctly predicting all 5 values (0, 120, 240, 360, 480).
- **Exercise 7.4 (Rendering the Markers)**: goal is creating a `marker-dash` div, positioning it from `markerY`, and appending it to `#game-track` inside the loop; a strong answer shows a real appended element each pass, and an explanation covering why `appendChild` is needed after `createElement`.
- **Exercise 7.5 (The Complete Marker System)**: goal is the COMPLETE marker system in one prompt; a strong answer shows the full loop combining every piece from this session, and an explanation stating the correct final values (0, 120, 240, 360, 480).

- **Homework Evaluation**: Ensure the Project Task's `for` loop spacing is computed from the student's own Session 3 track height (not a hardcoded 120px), appends the generated elements to `#game-track`, and carries no infinite-loop risk.
...
**Content note (2026-08-28):** 8.1-8.3 redesigned harder so function declarations, return values, and local-vs-global scope are each individually unmistakable, and 8.4 now combines every piece into one integrated system (still short of the Project Task's Controller-namespacing requirement, so the two don't overlap). 8.2/8.4 use a new `moveCount` counter (not `carX`) for the scope-shadowing demonstration: `buildJsSandboxPreview`'s harness makes `carX`/`speed` reactive globals and strips any `let/const/var carX` declaration anywhere in the pasted snippet (a text-level regex, not scope-aware) before running it, so a `carX`-shadowing bug would silently get neutralized by the harness and never actually reproduce live. Exercise 8.1's `clampPosition()` is a pure function with no DOM code at all, so it now gets a **Console Output**-only panel instead of the racing track (matching the S10/S12 precedent — a visible track that never moves reads as broken); 8.2-8.4 all clamp/assign `carX` and call `updatePlayerPosition()`, so they keep the **Live Racing Game Preview**:

- **Exercise 8.1 (The Return-Value Clamp Function)**: goal is a pure `clampPosition(x, min, max)` that **returns** `min`/`max`/`x` with no DOM code inside it, called with test values and logged to prove its return value; explanation contrasts a `return`-ending function (hands back a usable value) with `updatePlayerPosition()` (hands back `undefined`).
- **Exercise 8.2 (The Shadowed Counter Bug)**: goal is fixing a shadowing bug — `moveCount` re-declared with `let` inside `moveLeft()` silently creates a second, separate variable instead of sharing the outer one; explanation identifies the two-variables-one-name divergence via the exercise's two `console.log` calls.
- **Exercise 8.3 (Wiring the Return Value Back into Shared State)**: goal is `moveLeft()` assigning `clampPosition()`'s **returned** value back into the shared `carX` (`carX = clampPosition(carX - 130, 35, 295)`), wired to a complete keydown listener since this box runs standalone; explanation covers why an unassigned return value changes nothing.
- **Exercise 8.4 (Combining It All — Full Steering with Shared Helpers)**: goal is the complete system — `clampPosition()` shared by both `moveLeft()`/`moveRight()`, a single shared `moveCount` both increment correctly, the shared renderer, and one keydown listener for both arrow keys; explanation states only 1 place needs editing for a boundary change since the clamp helper and render call are reused, not duplicated.
...
- **Exercise 8.1 (The Return-Value Clamp Function)** — *Pure Function with a Return Value*
  ...
  - 💻 Reference Output Code: `function clampPosition(x, min, max) { if (x < min) return min; if (x > max) return max; return x; } console.log(clampPosition(10, 35, 295)); console.log(clampPosition(150, 35, 295)); console.log(clampPosition(400, 35, 295));`
- **Exercise 8.2 (The Shadowed Counter Bug)** — *Variable Shadowing: Local vs. Global Scope*
  ...
  - 💻 Reference Output Code: `/* carX is a global variable, already declared and managed by the sandbox itself (starts at 165) — that's why moveLeft() and updatePlayerPosition() below can both read/write it without declaring it here. */ let moveCount = 0; function updatePlayerPosition() { document.getElementById('player-car').style.left = carX + 'px'; } function moveLeft() { if (carX > 35) { carX -= 130; } moveCount++; updatePlayerPosition(); console.log('moveCount inside moveLeft():', moveCount); } moveLeft(); moveLeft(); console.log('moveCount outside moveLeft():', moveCount);`
- **Exercise 8.3 (Wiring the Return Value Back into Shared State)** — *Assigning a Function's Return Value into Global Scope*
  ...
  - 💻 Reference Output Code: `/* carX is a global variable, already declared and managed by the sandbox itself (starts at 165) — moveLeft() reassigns it directly (no let/const), so the one shared carX is what updatePlayerPosition() reads too. */ function clampPosition(x, min, max) { if (x < min) return min; if (x > max) return max; return x; } function updatePlayerPosition() { document.getElementById('player-car').style.left = carX + 'px'; } function moveLeft() { carX = clampPosition(carX - 130, 35, 295); updatePlayerPosition(); } window.addEventListener('keydown', function(event) { if (event.key === 'ArrowLeft') { moveLeft(); } });`
- **Exercise 8.4 (Combining It All — Full Steering with Shared Helpers)** — *Functions + Return Values + Shared Scope, Integrated*
  ...
  - 💻 Reference Output Code: `/* carX is a global variable, already declared and managed by the sandbox itself (starts at 165) — moveLeft()/moveRight() both reassign it directly (no let/const), so updatePlayerPosition() always renders the one shared value. */ let moveCount = 0; function clampPosition(x, min, max) { if (x < min) return min; if (x > max) return max; return x; } function updatePlayerPosition() { document.getElementById('player-car').style.left = carX + 'px'; } function moveLeft() { carX = clampPosition(carX - 130, 35, 295); moveCount++; updatePlayerPosition(); } function moveRight() { carX = clampPosition(carX + 130, 35, 295); moveCount++; updatePlayerPosition(); } window.addEventListener('keydown', function(event) { if (event.key === 'ArrowLeft') { moveLeft(); } else if (event.key === 'ArrowRight') { moveRight(); } });`
...
- Read and write the core JS syntax patterns this loop depends on: function declarations with `return`, the `if (!gameActive) return;` early-return guard, and how `let` declared inside vs. outside a function determines whether it resets or persists
...
2. **Core Concept Board Lesson: Chronological Time Deltas & Frame Rates (20 mins)**
   ...
   - *Topic 4 (JS Syntax Focus)*: Function declaration syntax — `function name(params) { ...; return value; }` — and how `return` hands a value back to the caller; the `if (!gameActive) return;` early-return guard, read line by line; the syntax difference between a `let x` written inside a function body (re-run, and reset, on every call) versus outside it (written once, keeps its value across every call).
...
5. **Socratic Debugging: The Ghost Frame Bug (15 mins)**
   - *Activity*: Tutor places the `gameActive` exit check AFTER the loop has already scheduled its next frame via `requestAnimationFrame`, so one extra "ghost" frame runs even after the game-over flag triggers true.
   - *Challenge*: Student moves the check to the very first line of the loop function, before anything else runs.
   - *Socratic Question*: *"The screen says Game Over, but why does the console show one more position update happening anyway? The exit check IS there — so why didn't it stop that frame?"*
...
**Content note (2026-09-03, 9.3 swapped for a genuinely recursive sample):** the wording fix above wasn't enough — the underlying SAMPLE CODE (`tick()`/`setTimeout`) was still the ambiguous "recursive setTimeout" mechanism, not the same recursion as 9.2. **9.3's code was replaced entirely** with `predictPosition(startY, speed, ticksRemaining)` — genuine synchronous, stack-based recursion (calls itself directly, builds and unwinds a real call stack, no `setTimeout` anywhere) that RETURNS a computed value, unlike 9.2's void-logging `tickRecursive()`. This is unambiguously the same recursion mechanism as 9.2, just more advanced (computing and handing back an answer instead of only printing), and it's thematically tied to the game (predicting the obstacle's future position). Since 9.3 no longer sets up self-scheduling at all, **9.4 now introduces `requestAnimationFrame` fresh and self-contained** — its Explain step asks why `gameLoop()` needs `requestAnimationFrame` instead of calling itself directly the way 9.2/9.3 did. **9.1-9.3 remain all DOM-free**, so all three keep the **Console Output**-only panel; only 9.4 keeps the **Live Racing Game Preview**.

- **Exercise 9.1 (The Loop Countdown)**: goal is a `for` loop logging a climbing count from 0 to 4, all running immediately, one after another — a Session 7 review and the baseline the next exercise contrasts against; explanation covers why all 5 log lines appear together, immediately, with no delay between repeats.
- **Exercise 9.2 (The Recursive Countdown)**: goal is the exact same 5-line output as 9.1, but via `tickRecursive(n)` calling ITSELF again (with an updated argument) instead of a loop, stopping once `n` reaches 4; explanation covers how the function's own parameter and stopping condition replace the loop's counter and condition.
- **Exercise 9.3 (The Recursive Predictor)**: goal is a recursive function `predictPosition(startY, speed, ticksRemaining)` that RETURNS the obstacle's position after that many more ticks — no loop, no `console.log` inside the function itself; explanation traces `predictPosition(0, 10, 3)` call by call (0→10→20→30, base case returns 30, which flows back up as the final answer). Unlike 9.2, this recursion hands back a real computed value — the genuine advanced form of the SAME mechanism as 9.2, not a different one.
- **Exercise 9.4 (Recursion in the Game)**: goal is the complete system — `nextObstaclePosition()` (a pure, return-value function, the same idea as 9.3's `predictPosition()` but one tick at a time), a genuinely-first `gameActive` guard, and a persisting `frameCount`, self-scheduling via `requestAnimationFrame` (introduced fresh here); explanation confirms calling `gameLoop()` directly again (like 9.2/9.3) would run instantly with no timing control and risk a runaway stack, while `requestAnimationFrame` waits for the current call to finish before scheduling the next.
...
- **Exercise 9.1 (The Loop Countdown)** — *For Loop Review — the Baseline Before Recursion*
  ...
  - 💻 Reference Output Code: `for (let i = 0; i <= 4; i++) { console.log('tickCount:', i); }`
- **Exercise 9.2 (The Recursive Countdown)** — *Same Task, Recursion Instead of a Loop*
  ...
  - 💻 Reference Output Code: `function tickRecursive(n) { console.log('tickCount:', n); if (n < 4) { tickRecursive(n + 1); } } tickRecursive(0);`
- **Exercise 9.3 (The Recursive Predictor)** — *Recursion That Returns a Computed Value (True Stack-Based Recursion)*
  ...
  - 💻 Reference Output Code: `function predictPosition(startY, speed, ticksRemaining) { if (ticksRemaining === 0) { return startY; } return predictPosition(startY + speed, speed, ticksRemaining - 1); } console.log(predictPosition(0, 10, 3)); console.log(predictPosition(50, 5, 10)); console.log(predictPosition(100, 20, 0));`
- **Exercise 9.4 (Recursion in the Game)** — *Return-Value Recursion + Self-Scheduling (requestAnimationFrame) + Guard-First, Applied to a Real Element*
  ...
  - 💻 Reference Output Code: `/* 9.3's predictPosition() returned a value computed by calling itself directly. gameLoop() does the same kind of return-value math (nextObstaclePosition), but instead of calling itself again immediately, it schedules its NEXT call via requestAnimationFrame — so it keeps running over time instead of just once. */ let obstacleY = -100; let gameActive = true; let score = 0; let frameCount = 0; function nextObstaclePosition(y, speed, resetY, wrapY) { const next = y + speed; if (next > wrapY) { return resetY; } return next; } function gameLoop() { if (!gameActive) return; const next = nextObstaclePosition(obstacleY, 20, -100, 500); if (next === -100) { score += 10; } obstacleY = next; document.getElementById('obstacle').style.top = obstacleY + 'px'; frameCount++; console.log('frameCount:', frameCount); requestAnimationFrame(gameLoop); } gameLoop();`
