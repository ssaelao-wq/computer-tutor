...
- **Exercise 9.1 (The Return-Value Position Stepper)**: goal is a pure `nextObstaclePosition(y, speed, resetY, wrapY)` that **returns** `y + speed` or `resetY` (once that would pass `wrapY`) with no DOM/loop code inside it, called with test values and logged to prove its return value; explanation covers why separating this math from the loop/DOM makes it independently testable.
- **Exercise 9.2 (The Ghost Frame Bug)**: goal is fixing an ordering bug — the `gameActive` guard is present but placed AFTER `requestAnimationFrame(gameLoop)` already re-queued a frame; the fix moves it to the very first line of `gameLoop()`; explanation covers why checking late still lets one "ghost frame" run.
- **Exercise 9.3 (Wiring the Return Value into a Persisting Loop)**: goal is `gameLoop()` assigning `nextObstaclePosition()`'s **returned** value to `obstacleY` (moving `#obstacle`, bumping `score` on wrap), guarded on `gameActive`, plus a `frameCount` declared OUTSIDE the function so it survives across recursive calls; explanation covers why a `frameCount` declared inside would reset every frame instead of climbing.
- **Exercise 9.4 (Combining It All — The Complete Animation Engine)**: goal is the complete system — `nextObstaclePosition()` shared with the loop, a genuinely-first `gameActive` guard, and a persisting `frameCount`, all self-scheduling via `requestAnimationFrame`; explanation confirms a guard placed after the `frameCount` increment would still let it run one extra time after Game Over.
