...
### What is actually built and running (snapshot through 2026-07-30 — see correction below for current state)
...
- **Bug fixes found by real testing:** Live Preview console/verification log split; preview height (260px→360px); Exercise 5.1 scope gap; L1S5 Project Task duplicated its own exercises (same "steer left/right" objective as 5.1-5.4). First fix (a held-key auto-repeat bug) was rejected — teacher didn't want a "find a bug" framing, wanted genuinely different *behavior* instead. Rebuilt around on-screen ◀/▶ buttons that trigger the same steering as the keyboard, forcing the student to factor movement into one shared action instead of duplicating it — ties into the session's own accessibility Ethics discussion. Verified end-to-end with a headless-browser run (caught and fixed a real bug in the process: the sample code assumed the buttons already existed instead of creating them via `document.createElement`, since this session has no HTML tab). Teacher Guideline synced to match.
- **L1S6 Project Task had the same duplication, one layer deeper:** its "speed overheat clamp the sandbox drill doesn't cover" was factually wrong — Exercise 6.4 already teaches that exact clamp. Fixed by making the real differentiator *where* the boundary guard lives: inside the shared `steerCar()` function from S5's Project Task, so it protects both the keyboard AND the on-screen buttons — something no S6 exercise even mentions, since none of them know the buttons exist. Teacher Guideline synced to match.
- **S7–S12 and all of L2 are untouched** — original keyword `validate()`, 5 exercises each. *(Stale as of 2026-08-21 — see correction directly below. Left in place as the historical record of what this Part I diagnosed at the time.)*

> **Correction (2026-08-21):** the "S7–S12 untouched" claim above is no longer true. S8–S12 moved to the Auditor on 2026-08-05 (`[[project_l1s6_s12_preview_harness_gap_2026_08_05]]`), and S7 — the last holdout, kept on keyword `validate()` even after everything else moved on — was finally migrated too, after a real student hit it as "guessing the exact wording" (`[[project_l1s7_auditor_grading_fix_2026_08_21]]`). **All twelve L1 sessions (S1–S12) are now on Auditor-graded, prompt-quality-only grading — this is the actual current state, not just S1–S6.** The same audit also found and fixed two more bug classes across most of L1 (S4–S6, S8–S12): a "Verify feedback" panel positioned above the Verify button so results were scrolled out of view (`[[project_l1s8_s12_bug_audit_2026_08_21]]`), and a Generate Code button that stole keyboard focus to the preview iframe on sessions with no steering interaction. **L2 remains genuinely untouched** — same keyword `validate()`, confirmed still true (`[[project_l2_keyword_grading_deferred_2026_08_21]]`), except one narrow fix to Session 11's dead preview panel (`[[project_l2s11_dead_preview_fix_2026_08_21]]`), which did not touch its grading.
