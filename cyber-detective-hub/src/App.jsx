...
const S7_EXERCISES = [
  ...
  },
  {
    ...
    problem: "This sandbox already tracks a 'speed' value that starts at 0, the same way carX already starts at 165 in earlier sessions — it's provided by the sandbox, not something you declare yourself. A real car can't accelerate forever, and it doesn't stop dead the instant you let off the gas either — it coasts back down gradually. Right now nothing caps speed, and nothing happens when it gets dangerously high. Real racing games flash a warning color once a car crosses a speed threshold — here, that threshold is 100, and the maximum speed is 120.",
    instruction: "Goal: holding ArrowUp should speed the car up (capped at 120, never higher); releasing it should let speed coast back down slowly toward 0 instead of freezing in place. On every check, use an if/else to change #game-track's border color to red once speed has reached the 100 danger threshold (yellow otherwise). This needs the same 'is the key currently held' tracker Session 6.4 used (a flag set true on keydown, false on keyup) plus something that keeps running on its own (like setInterval) so the car can coast down even after the key event is long over — while held, use a for loop of a few small pulses to raise speed each tick, capped at 120; while not held, lower speed a little each tick instead, floored at 0. Since speed already exists in this sandbox (starting at 0), do NOT declare it again. 1) Plan: how will you track 'currently held', how much does speed change per tick while accelerating vs. coasting, and where do the 120 cap and the 100 color threshold fit in? 2) Prompt: describe that goal to the AI, in your own words. 3) Explain: what happens to speed if you hold ArrowUp well past 120, and what happens to the border if you let go while it's still red?",
    planPlaceholder: "Plan: how will you track 'currently held', how much does speed change per tick while accelerating vs. coasting, and where do the 120 cap and 100 color threshold fit in?",
    promptPlaceholder: "Describe the goal to the AI, in your own words — hold ArrowUp to speed up (capped at 120) via a for loop of pulses, release to coast down slowly, with an if/else turning the track border red at 100.",
    outputCodePlaceholder: "Paste the actual code the AI generated from your prompt here.",
    runnable: true,
    expectedConcepts: "speed already exists in this sandbox starting at 0 (like carX already exists at 165) — a strong answer does NOT redeclare it. The code should track whether ArrowUp is currently held (a boolean flag or similar, declared once, set true on keydown and false on keyup — not just referenced without ever being updated, the same trap Session 6.4 warned about) and use something that keeps running on its own (setInterval or requestAnimationFrame) so speed keeps changing even when no key event is firing. While held, each tick should run a for loop of several pulses adding to speed, capped so it never exceeds 120. While not held, speed should decrease a little each tick, floored so it never goes below 0 — this is the 'coast down slowly' behavior, not an instant snap to 0. On every tick, an if/else should check whether speed has reached 100: if so, set #game-track's border color to red; otherwise reset it to yellow. The explanation should correctly say speed stops climbing once it hits the 120 cap (holding longer doesn't push it higher), and that releasing ArrowUp while the border is red doesn't instantly turn it yellow again — the border stays red until the gradually-decaying speed actually drops back under 100. Don't require exact numbers or timing in the plan/prompt prose — judge whether the reasoning correctly connects held-vs-released state, the for loop's pulses, the 120 cap, and the gradual coast-down. Don't require the literal code in the plan/prompt prose — judge the reasoning."
  }
];
...
export default function App() {
  ...
  const s9OutputCodeRef = useRef(null);
  ...
  const s10OutputCodeRef = useRef(null);
  ...
  const s12OutputCodeRef = useRef(null);
                  ...
                  <div className="glass-panel" style={{ padding: '16px' }}>
                    ...
                    <div className="sim-panel-body" style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '10px 0 0' }}>
                      {/* All 4 exercises (2026-08-21 redesign) render real visible output —
                          ...
                          preview is always shown. 7.1-7.3 don't touch carX, so the car stays
                          hidden there; 7.4 is specifically about the CAR crossing a speed
                          threshold via real ArrowUp presses, so it's the one exercise in this
                          session that shows the car and needs autoFocus for keyboard input. */}
                      <iframe
                        srcDoc={buildJsSandboxPreview(
                          ...
                          { showCar: s7ActiveExercise === 4, trackHeight: 660, autoFocus: s7ActiveExercise === 4 }
                        )}
                          ...
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                setSimConsoleLogs([]);
                                // This session has no arrow-key/space interaction, so keep focus on
                                // the Output Code box instead of the iframe (see S7's identical fix).
                                s9OutputCodeRef.current?.focus();
                              } catch (err) {
                          ...
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                // This session has no arrow-key/space interaction, so keep focus on
                                // the Output Code box instead of the iframe (see S7's identical fix).
                                s10OutputCodeRef.current?.focus();
                              } catch (err) {
                          ...
                          <button
                            ...
                            onClick={async () => {
                              ...
                              try {
                                ...
                                // This session has no arrow-key/space interaction, so keep focus on
                                // the Output Code box instead of the iframe (see S7's identical fix).
                                s12OutputCodeRef.current?.focus();
                              } catch (err) {
