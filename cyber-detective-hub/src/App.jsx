...
const S7_EXERCISES = [
  ...
  },
  {
    ...
    problem: "This sandbox already tracks a 'speed' value that starts at 0, the same way carX already starts at 165 in earlier sessions — it's provided by the sandbox, not something you declare yourself. Nothing currently happens when speed gets dangerously high. Real racing games flash a warning color once a car crosses a speed threshold — here, that threshold is 100 — and that change needs to be something you actually watch happen, not something that's already finished the instant the code runs.",
    instruction: "Goal: use a for loop to SCHEDULE a series of delayed speed increases (via setTimeout, one per loop pass, each a bit later than the last) so the car visibly accelerates over a few seconds instead of jumping straight to its final speed — and on each scheduled update, check with an if/else whether speed has reached the 100 danger threshold: if it has, change #game-track's border color to red to warn the player; otherwise leave it at its normal yellow. Since speed already exists in this sandbox (starting at 0), do NOT declare it again — just increase the existing speed on each scheduled update, enough times that it actually crosses 100 before the last one. 1) Plan: how many scheduled updates, how far apart in time, and how much should speed increase each time (enough to pass 100 by the end)? 2) Prompt: describe that goal to the AI, in your own words — make sure it asks for the updates to be delayed/staggered, not instant. 3) Explain: at what update (and what speed value) did the road color actually change, and why couldn't a plain, non-delayed for loop show this happening?",
    planPlaceholder: "Plan: how many scheduled updates, how far apart in time, and how much does speed increase each time (enough to pass 100 by the end)?",
    promptPlaceholder: "Describe the goal to the AI, in your own words — a for loop scheduling delayed speed increases (already provided, starting at 0) via setTimeout, with an if/else that turns the track border red once speed reaches 100.",
    outputCodePlaceholder: "Paste the actual code the AI generated from your prompt here.",
    runnable: true,
    expectedConcepts: "speed already exists in this sandbox starting at 0 (like carX already exists at 165) — a strong answer does NOT redeclare it with let/const/var, it just increases the existing value. A for loop should be used to SCHEDULE several delayed updates (e.g. via setTimeout with each iteration's delay increasing, like frame * 300), not run synchronously — a plain for loop with no delay would resolve to the final speed instantly, with nothing to actually watch. Each scheduled update should increase speed and then check with an if/else whether it has reached 100: if so, set #game-track's border color to red; otherwise reset it to yellow. The explanation should correctly identify which scheduled update (and what speed value) first crossed 100 and changed the color, and should correctly say a non-delayed for loop can't show a visible progression because it finishes all its passes in the same instant. Don't require exact timing/frame counts in the plan/prompt prose — judge whether the reasoning correctly connects delayed scheduling to a visibly-watchable threshold crossing, and correctly avoids redeclaring speed. Don't require the literal code in the plan/prompt prose — judge the reasoning."
  }
];
                  ...
                  <div className="glass-panel" style={{ padding: '16px' }}>
                    ...
                    <div className="sim-panel-body" style={{ display: 'flex', flexDirection: 'column', gap: '10px', padding: '10px 0 0' }}>
                      {/* All 4 exercises (2026-08-21 redesign) render real visible output —
                          ...
                          threshold, so it's the one exercise in this session that shows it. */}
                      <iframe
                        srcDoc={buildJsSandboxPreview(
                          ...
                          { showCar: s7ActiveExercise === 4, trackHeight: 660, autoFocus: false }
                        )}
