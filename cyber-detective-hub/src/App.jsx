...
const S7_EXERCISES = [
  ...
  },
  {
    ...
    problem: "This sandbox already tracks a 'speed' value that starts at 0, the same way carX already starts at 165 in earlier sessions — it's provided by the sandbox, not something you declare yourself. Nothing currently happens when speed gets dangerously high. Real racing games flash a warning color once a car crosses a speed threshold — here, that threshold is 100 — and the player should be able to make it happen themselves by holding the accelerator, not just watch a scripted animation play once.",
    instruction: "Goal: make pressing ArrowUp speed the car up in bursts, and use an if/else to change #game-track's border color to red once speed reaches the 100 danger threshold (yellow otherwise). Each ArrowUp press should run a for loop of several small pulses that each add a bit of speed — a for loop is the natural way to repeat 'one more pulse' a few times per press, rather than one bare addition. Since speed already exists in this sandbox (starting at 0), do NOT declare it again — just increase it. Click inside the Preview, then press ArrowUp repeatedly to test it live and watch the border change. 1) Plan: how many pulses per press, how much speed per pulse, and what should the if/else do in each branch? 2) Prompt: describe that goal to the AI, in your own words. 3) Explain: how many ArrowUp presses did it actually take before the border turned red, and why?",
    planPlaceholder: "Plan: how many pulses per ArrowUp press, how much speed per pulse, and what does the if/else do in each branch?",
    promptPlaceholder: "Describe the goal to the AI, in your own words — pressing ArrowUp runs a for loop of a few speed pulses (speed already exists, starting at 0), with an if/else that turns the track border red once speed reaches 100.",
    outputCodePlaceholder: "Paste the actual code the AI generated from your prompt here.",
    runnable: true,
    expectedConcepts: "speed already exists in this sandbox starting at 0 (like carX already exists at 165) — a strong answer does NOT redeclare it with let/const/var, it just increases the existing value. A keydown handler listening for ArrowUp should, on each press, run a for loop of several pulses (e.g. 5), each adding to speed, so one press produces a real, for-loop-driven jump rather than a single bare addition. After the loop, an if/else should check whether speed has reached 100: if so, set #game-track's border color to red; otherwise reset it to yellow. The explanation should correctly report how many ArrowUp presses it took to cross 100, consistent with whatever per-press increase their own code actually used. Don't require exact numbers in the plan/prompt prose — judge whether the reasoning correctly connects real key-press interaction, the for loop's repeated pulses, and the conditional border change. Don't require the literal code in the plan/prompt prose — judge the reasoning."
  }
];
