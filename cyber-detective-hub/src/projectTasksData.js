...
export const PROJECT_TASKS = {
  ...
  "l1-s6": {
    partNum: "Lab 6",
    partTitle: "Safety Guards & the Full Control System",
    objectives: [
      "Extend Session 5's game.js — including its steerCar() function and ◀/▶ buttons — don't rewrite it from zero",
      "Construct left/right limit guards from your own track width (not hardcoded numbers), inside steerCar() itself, so BOTH the keyboard AND the ◀/▶ buttons respect them",
      "Add two new on-screen buttons, ▲ Accelerate and ▼ Brake, alongside the existing ◀/▶",
      "Build ONE shared function (e.g. changeSpeed()) containing the full speed system from Exercises 6.3-6.4 — the accelerate/decelerate range, the overheat reset to a Number, and the reverse handoff — reused by BOTH the keyboard's ArrowUp/ArrowDown AND the two new buttons",
      "Make that function update the live speed number on screen and scroll the track's background, so the finished milestone visibly looks different — not just 'doesn't crash at the edges'"
    ],
    planSpecs: {
      vision: "Same steering car — but now the FULL control system is protected and complete: left/right steering can't go off the road, and speed has its own real range (0 up to 120, with an overheat reset) plus a reverse gear once fully braked — all reachable by keyboard OR by clicking on-screen buttons, identically. Session 5 gave you ◀/▶ for left/right; this session adds ▲/▼ for accelerate/brake, following the exact same shared-function pattern.",
      parts: "Goal: work out, in your own words, WHERE each guard has to live so it protects both input paths at once — steerCar() for left/right, and a new shared function for speed — not copied separately into the keydown handler and each button's click handler. Then work out how your speed function should behave differently depending on whether speed is already 0 (brake vs. reverse, from Exercise 6.4), and how to calculate the real left/right limits from your own TRACK_WIDTH and car width.",
      flow: "Goal: describe your own step-by-step logic for steerCar(direction) (checking the limit BEFORE moving carX) AND for your new speed function (checking whether to accelerate, brake, or reverse depending on the current speed) — for whichever direction was requested, regardless of whether a keypress or a button click triggered the call. Work it out yourself before writing any code."
    },
    chainFrom: "l1-s5",
    promptGuide: "Write your own prompt for the full control system you planned above, in your own words. Make sure it tells the AI to: (1) add the left/right check INSIDE your existing steerCar() function, deriving the limits from your own TRACK_WIDTH/car width; (2) create two new buttons, ▲ Accelerate and ▼ Brake, styled like your existing ◀/▶; and (3) build ONE shared speed function containing the full accelerate/decelerate/overheat/reverse system from Exercises 6.3-6.4, called by BOTH your keyboard handler's ArrowUp/ArrowDown cases AND the two new buttons — including updating the live speed number on screen and scrolling the track's background.",
    codeReviewGuide: [
      "Is the left/right boundary check written once, inside steerCar() itself, so both the keyboard and the ◀/▶ buttons are protected the same way?",
      "Do the left/right limits come from your own TRACK_WIDTH/car width, not a hardcoded 35/295?",
      "Is there ONE shared speed function — not duplicated logic in the keydown handler and each button — containing the accelerate cap, decelerate floor, and reverse handoff from Exercises 6.3-6.4?",
      "Does the speed overheat reset use a Number, not a quoted string (reusing what you fixed in Exercise 6.3)? And do the new ▲/▼ buttons actually exist as clickable elements that call that exact same function?",
      "Socratic Question: if a guard were only added inside the keydown handler and not the shared function, what would happen the moment a player switched from holding a key to clicking its matching button mid-action?"
    ],
    sampleGeneratedHtml: '// game.js — full control system: left/right steering (from 6.1-6.2) AND\n// accelerate/brake/reverse (from 6.3-6.4), each reachable by BOTH the\n// keyboard and its own on-screen button, through one shared function per axis.\n// (carX, LANE_WIDTH, TRACK_WIDTH, the keydown handler, and the ◀/▶ buttons\n// that already call steerCar() all exist above this — not repeated here)\nconst LEFT_LIMIT = (TRACK_WIDTH % LANE_WIDTH === 0) ? 0 : Math.floor((TRACK_WIDTH % LANE_WIDTH) / 2);\nconst RIGHT_LIMIT = TRACK_WIDTH - LANE_WIDTH - LEFT_LIMIT;\nconst MAX_SPEED = 120;\n\nfunction steerCar(direction) {\n  if (direction === "left" && carX > LEFT_LIMIT) {\n    carX -= LANE_WIDTH;\n  } else if (direction === "right" && carX < RIGHT_LIMIT) {\n    carX += LANE_WIDTH;\n  } else {\n    return; // blocked at the edge — keyboard AND buttons both stop here\n  }\n  document.getElementById("player-car").style.left = carX + "px";\n}\n\nlet roadOffset = 0;\n\nfunction changeSpeed(direction) {\n  if (direction === "up") {\n    speed += 10;\n    if (speed > MAX_SPEED) {\n      speed = 100; // Number, not "100" — the Session 4 bug\n      console.warn("Overheat! Speed clamped to", speed);\n    }\n    roadOffset += speed;\n  } else if (direction === "down") {\n    if (speed > 0) {\n      speed = Math.max(0, speed - 10); // braking\n    } else {\n      roadOffset = Math.max(0, roadOffset - 15); // already stopped -> reverse instead\n    }\n  }\n  document.getElementById("speed-val").textContent = speed;\n  document.getElementById("game-track").style.backgroundPositionY = roadOffset + "px";\n}\n\n// extend your existing keydown handler with two more branches:\nwindow.addEventListener("keydown", function(event) {\n  if (event.key === "ArrowLeft") {\n    steerCar("left");\n  } else if (event.key === "ArrowRight") {\n    steerCar("right");\n  } else if (event.key === "ArrowUp") {\n    changeSpeed("up");\n  } else if (event.key === "ArrowDown") {\n    changeSpeed("down");\n  }\n});\n\n// mirrors Session 5\'s ◀/▶ buttons — same shared-function pattern, new axis\nconst upButton = document.createElement("button");\nupButton.id = "btn-accelerate";\nupButton.textContent = "\\u25B2";\nupButton.addEventListener("click", function() {\n  changeSpeed("up");\n});\ndocument.body.appendChild(upButton);\n\nconst downButton = document.createElement("button");\ndownButton.id = "btn-brake";\ndownButton.textContent = "\\u25BC";\ndownButton.addEventListener("click", function() {\n  changeSpeed("down");\n});\ndocument.body.appendChild(downButton);',
    testCasesGuide: "- Press ArrowLeft repeatedly to the left edge: verify the car locks at your own left limit\n- At that same left edge, click the ▶ button then ◀ again: verify clicking behaves identically to the keyboard at the boundary\n- Hold ArrowUp (or click ▲ repeatedly): verify speed climbs, #speed-val updates, and it clamps to a Number at your MAX_SPEED\n- Then hold ArrowDown (or click ▼ repeatedly): verify speed brakes back down to 0, and further presses/clicks start decreasing roadOffset instead — clamped at 0",
    iterationGuide: "If clicking a button produces different behavior than its matching key (for steering OR for speed), that's a sign a guard or the speed function was added to only one input path instead of the shared function — move it there. If speed or roadOffset ever goes negative, the floor check is missing or in the wrong branch.",
    targetOutcomeHtml: '<div id="dashboard"><h2>Score: <span id="score-val">0</span> | Speed: <span id="speed-val">40</span></h2></div><div id="game-track"><div id="player-car" style="left: 2%;"></div><div class="lane-divider"></div></div><div id="control-buttons"><button id="btn-left">◀</button><button id="btn-accelerate">▲</button><button id="btn-brake">▼</button><button id="btn-right">▶</button></div>',
    targetOutcomeCaption: "Boundary clamps stop the car from sliding off the track, and the live Speed readout plus the scrolling road background make acceleration and braking actually visible — all from the keyboard OR the four on-screen buttons, since every guard lives inside one shared function per axis. Still Session 4's file, still growing."
  },
  "l1-s7": {
    partNum: "Lab 7",
