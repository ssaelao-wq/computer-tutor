...
export const PROJECT_TASKS = {
  ...
  "l1-s6": {
    partNum: "Lab 6",
    partTitle: "Safety Guards & Multi-Input Controls (Keyboard + Mouse)",
    objectives: [
      "Extend Session 5's game.js — including its steerCar() function and ◀/▶ buttons — don't rewrite it from zero",
      "Construct left and right limit guards (from Exercises 6.1 & 6.2) inside steerCar() itself, so BOTH the keyboard (ArrowLeft/ArrowRight) AND the mouse click buttons (◀/▶) keep the car safely on the road",
      "Implement the speed acceleration and momentum system (from Exercise 6.4) up to a cap of 300, advancing roadOffset to scroll the track background, controllable by BOTH the ArrowUp key and the ▲ on-screen button",
      "Support dual input modes: keyboard controls (ArrowLeft, ArrowRight, ArrowUp) AND mouse click buttons (◀ Left, ▶ Right, ▲ Accelerate), ensuring each pair triggers the exact same shared logic",
      "Update the live speed readout in the HUD (#speed-val) and advance roadOffset to scroll the road background (#game-track) as the car speeds up"
    ],
    planSpecs: {
      vision: "A complete multi-input racing car game control system: steer left (6.1) and right (6.2) with safety boundary guards preventing the car from running off the track, and accelerate up to 300 with forward road scrolling (6.4) — all fully playable with either keyboard (ArrowLeft, ArrowRight, ArrowUp) OR mouse clicks (◀ Left, ▶ Right, ▲ Accelerate buttons).",
      parts: "Goal: work out, in your own words, WHERE each guard and control function has to live so both input methods (keyboard and mouse buttons) call the same shared logic — steerCar(direction) for left/right boundary checks (carX > LEFT_LIMIT and carX < RIGHT_LIMIT), and an acceleration/speed function to increase speed up to 300, advance roadOffset, and update #speed-val. Make sure each mouse button (◀, ▶, ▲) and keyboard key (ArrowLeft, ArrowRight, ArrowUp) triggers the identical shared function.",
      flow: "Goal: describe your own step-by-step logic: (1) When Left is triggered (ArrowLeft or ◀ button), check if carX > LEFT_LIMIT before subtracting lane position; (2) When Right is triggered (ArrowRight or ▶ button), check if carX < RIGHT_LIMIT before adding lane position; (3) When Up is triggered (ArrowUp or ▲ button), increase speed up to 300, advance roadOffset to scroll the track background, and update #speed-val in HUD; (4) When not accelerating, allow speed to coast down smoothly."
    },
    chainFrom: "l1-s5",
    promptGuide: "Write your own prompt for the full control system you planned above, in your own words. Make sure it tells the AI to: (1) add the left/right boundary checks INSIDE your existing steerCar() function (from Exercises 6.1 & 6.2); (2) implement speed acceleration up to 300 and roadOffset background scrolling (from Exercise 6.4); (3) wire the keyboard handler for ArrowLeft, ArrowRight, and ArrowUp; and (4) wire mouse click listeners for the three on-screen buttons (◀ Left, ▶ Right, and ▲ Accelerate) so clicking a button triggers the exact same logic as pressing its keyboard arrow key.",
    codeReviewGuide: [
      "Is the left/right boundary check written once inside steerCar() so both the keyboard keys and the ◀/▶ buttons are protected against steering off-track?",
      "Does the acceleration logic cap speed at 300 and advance roadOffset to visibly scroll the track?",
      "Are all three controls accessible via BOTH keyboard (ArrowLeft, ArrowRight, ArrowUp) AND mouse click buttons (◀, ▶, ▲)?",
      "Do the button click event listeners reuse the exact same functions as the keydown handler rather than duplicating movement logic?",
      "Socratic Question: Why is it better to call steerCar('left') from both the key listener and the button listener, instead of copying the carX code into both places?"
    ],
    sampleGeneratedHtml: '// game.js — full control system: left/right boundary steering (6.1-6.2)\n// and speed acceleration (6.4), accessible by BOTH keyboard and mouse buttons.\nconst LEFT_LIMIT = 35;\nconst RIGHT_LIMIT = 295;\nconst MAX_SPEED = 300;\n\nlet roadOffset = 0;\n\nfunction steerCar(direction) {\n  if (direction === "left" && carX > LEFT_LIMIT) {\n    carX -= 130;\n  } else if (direction === "right" && carX < RIGHT_LIMIT) {\n    carX += 130;\n  }\n  document.getElementById("player-car").style.left = carX + "px";\n}\n\nfunction accelerate() {\n  if (speed < MAX_SPEED) {\n    speed = Math.min(MAX_SPEED, speed + 10);\n  }\n  roadOffset += speed;\n  document.getElementById("speed-val").textContent = speed;\n  document.getElementById("game-track").style.backgroundPositionY = roadOffset + "px";\n}\n\n// Keyboard controls: ArrowLeft, ArrowRight, ArrowUp\nwindow.addEventListener("keydown", function(event) {\n  if (event.key === "ArrowLeft") {\n    steerCar("left");\n  } else if (event.key === "ArrowRight") {\n    steerCar("right");\n  } else if (event.key === "ArrowUp") {\n    accelerate();\n  }\n});\n\n// Mouse click buttons: ◀ Left, ▶ Right, ▲ Accelerate\nconst leftBtn = document.getElementById("btn-left") || document.createElement("button");\nleftBtn.id = "btn-left";\nleftBtn.textContent = "◀";\nleftBtn.addEventListener("click", function() {\n  steerCar("left");\n});\n\nconst rightBtn = document.getElementById("btn-right") || document.createElement("button");\nrightBtn.id = "btn-right";\nrightBtn.textContent = "▶";\nrightBtn.addEventListener("click", function() {\n  steerCar("right");\n});\n\nconst upBtn = document.getElementById("btn-accelerate") || document.createElement("button");\nupBtn.id = "btn-accelerate";\nupBtn.textContent = "▲";\nupBtn.addEventListener("click", function() {\n  accelerate();\n});',
    testCasesGuide: "- Press ArrowLeft repeatedly (or click ◀): verify the car locks at the left boundary (35px) and cannot go off-road\n- Press ArrowRight repeatedly (or click ▶): verify the car locks at the right boundary (295px) and cannot go off-road\n- Press ArrowUp (or click ▲): verify speed climbs toward 300, #speed-val updates in the HUD, and the track background scrolls forward\n- Test mouse clicks: click ◀, ▶, and ▲ to confirm they execute the exact same movement and acceleration as the arrow keys",
    iterationGuide: "If clicking a button produces different behavior than its matching key (for steering OR for speed), that's a sign the logic was duplicated into one handler instead of calling the shared function — refactor so both call the same function. If speed goes above 300 or below 0, check your clamping guards.",
    targetOutcomeHtml: '<div id="dashboard"><h2>Score: <span id="score-val">0</span> | Speed: <span id="speed-val">40</span></h2></div><div id="game-track"><div id="player-car" style="left: 165px;"></div><div class="lane-divider"></div></div><div id="control-buttons"><button id="btn-left">◀</button><button id="btn-accelerate">▲</button><button id="btn-right">▶</button></div>',
    targetOutcomeCaption: "Boundary clamps stop the car from sliding off the track, and the live Speed readout plus scrolling track background make acceleration visible — all controllable from keyboard arrow keys (Left, Right, Up) OR mouse clicking the three on-screen buttons (◀, ▶, ▲)."
  },
  "l1-s7": {
    partNum: "Lab 7",
