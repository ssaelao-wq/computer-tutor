...
- **Level themes must stay internally consistent**: L1 Racing Car Game, L2 Mars Colony Defense, L3 Cyberpunk Hacker Arena, L4 Mission Control Dashboard. All sessions within a level share one theme; content must stay age-appropriate/adventurous (avoid domestic/baby-ish framing). Check `CURRICULUM_DATA` in `src/curriculumData.js` against this when adding sessions.
- **Code review mechanism is intentionally level-gated**: L1 = copy-paste (deliberate friction, not a shortcut to fix), L2 = platform-run `git diff` against student's repo, L3 = GitHub PR diff, L4 = CI/CD-triggered audit. Don't "improve" L1 by adding auto-diffing — the copy-paste friction is pedagogically intentional.
